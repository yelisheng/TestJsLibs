...
var rdsClient = redis.createClient(redisPort, redisHost);
...
var FixtureSite = (function () {
  ...
  FixtureSite.prototype.initExpressSite = function () {
    ...
    //test redis
    app.get('/redis/test', function (req, res, next) {
      rdsClient.incr('counter', function (err, result) {
        if (err) return next(err);
        res.json({ title: 'redis记录刷新次数', count: result });
      });
    });
    ...
    app.get('/testform', function (req, res) {
      res.setHeader('Content-Type', 'text/html');
      res.write(
        "<html><body><form id='test' action='/formpost' method='post'>\
...
function makeid() {
  ...
  for (var i = 0; i < 255; i++) text += possible.charAt(Math.floor(Math.random() * possible.length));
  return text;
}
